...
const trendingRepositories = [
  ...
  },
  {
    ...
    "language": "Rust",
    "stars": 14086,
    "purpose": "一款本地优先、跨 Windows、macOS 和 Linux 的 Logitech 外设配置应用，目标是提供 Logitech Options+ 的开源替代方案。它可通过 HID++ 管理按键映射、鼠标 DPI、SmartShift、灯光以及按应用切换的配置文件，且不要求 Logitech 账户。不同型号和操作系统的功能覆盖并不完全相同，采用前需要核对兼容设备列表。",
  ...
  },
  {
    ...
    "language": "Python",
    "stars": 32143,
    "purpose": "面向 AI Agent 的上下文数据库，把长期记忆、知识资料和技能组织成可导航的虚拟文件系统。它通过分层加载、检索、会话捕获与提交机制，让 Agent 在多次会话之间复用必要上下文，并提供服务端、CLI、SDK 和 MCP 接口。官方还提供 Codex、Claude Code、OpenClaw、Cursor、DeepSeek Harness 等集成，适合需要统一管理 Agent 上下文的工程。",
  ...
  },
  {
    ...
    "language": "TypeScript",
    "stars": 7137,
    "purpose": "一个以 TypeScript 编写的插件元框架和运行时，用 Context、Service、依赖注入、类型事件及生命周期管理来组合可插拔能力。DeepSeek Harness 将模型适配器、工具、文件访问、Agent Loop、存储和界面都组织为 Cordis 插件，因此它主要服务于需要替换、扩展或动态装卸模块的框架开发者，而不是可直接使用的终端应用。",
  ...
  },
  {
    ...
    "language": "Shell",
    "stars": 28407,
    "purpose": "由 37signals 孵化、基于 Arch Linux 的强观点开发者发行版与配置系统。它预装并统一配置 Hyprland、终端、编辑器、浏览器、截图、剪贴板和 AI 编程工具，通过 ISO、安装脚本和手册快速搭建一致的键盘优先开发环境。它强调预设工作流，未必适合需要保守更新策略或大量自定义桌面组件的用户。",
  ...
  },
  {
    ...
    "language": "Rust",
    "stars": 4116,
    "purpose": "一个为 AI 编程代理提供跨会话、跨客户端长期记忆的独立 Rust 服务。它通过 MCP 和生命周期钩子自动记录提示、工具调用与决策，在会话结束后整理成以 Git 管理的 Markdown 知识库，并在下一次 Claude Code、Codex、Cursor、OpenCode 等会话开始时注入交接摘要。SQLite 仅作为全文检索和索引层，Markdown 才是可人工编辑、可备份和可审计的事实来源。",
  ...
  },
  {
    ...
    "language": "Python",
    "stars": 10303,
    "purpose": "一套面向可审计 AI 系统的图原生上下文基础设施。它可从文档、数据库和 API 摄取信息，完成实体抽取、消歧、合并与知识图谱构建，并支持本体、SHACL 校验、规则推理、来源追踪和决策记录。项目提供 Python 包、Knowledge Explorer、CLI、REST API 与 MCP Server，适合需要可解释、可追溯知识上下文的 Agent 或治理系统，但不等于暴露模型内部思维链。",
  ...
  },
  {
    ...
    "language": "Mojo",
    "stars": 28875,
    "purpose": "Modular AI 开发与部署平台的开源代码仓库，主要包含 MAX 框架和 Mojo 编程语言。MAX 用于在 CPU、NVIDIA 或 AMD GPU 上运行和部署生成式 AI 模型，提供模型管线、推理服务器、OpenAI 兼容 REST 接口和 Kubernetes 可用容器；Mojo 用于编写高性能 CPU/GPU 内核，并支持与 Python 互操作。仓库主分支跟随 nightly 构建，生产使用应选择对应的稳定发布分支并单独核对 Modular 及第三方模型许可证。",
    "example": "本次自动刷新未找到可公开核实的真实网站、App、下游产品或第三方采用案例。",
    "risk": "采用前需核实许可证、安全策略、维护活跃度、版本兼容性和生产环境支持情况。",
    "weekly": 2017
  }
